...
async def get_chat_list(
    ...
    other_user_expr = case(
        (Message.sender_id == current_user.id, Message.receiver_id), else_=Message.sender_id
    )
    ...
    subquery = (
        select(
            ...
            func.count(
                case(
                    (
                        and_(Message.receiver_id == current_user.id, ~Message.is_read),
                        Message.id,
                    ),
                    else_=None,
                )
    ...
    result = await db.execute(
        select(User, subquery.c.last_message_time, subquery.c.unread_count)
    ...
    chat_list = []
    for user, _last_message_time, unread_count in rows:
        # Get last message
        ...
        contact_result = await db.execute(
            select(Contact).where(Contact.user_id == current_user.id, Contact.contact_id == user.id)
        )
